...
// Clinic WhatsApp number in international format (no +, spaces or dashes).
  ...
  return (
    <div className="fixed bottom-4 right-4 z-40 flex flex-col items-end gap-3 md:hidden">
      ...
      <button
        onClick={() => open()}
        aria-label={c.book}
        className="flex items-center gap-2 rounded-full bg-gradient-to-r from-coral-500 to-coral-600 py-3 pl-4 pr-5 text-sm font-bold text-white shadow-lg shadow-coral-500/40 active:scale-95"
      >
        <CalendarIcon />
        {c.book}
      </button>
      ...
      <a
        ...
        className="flex items-center gap-2 rounded-full bg-[#25D366] py-3 pl-4 pr-5 text-sm font-bold text-white shadow-lg shadow-[#25D366]/40 active:scale-95"
      >
        <WhatsAppIcon />
        {c.whatsapp}
      </a>
